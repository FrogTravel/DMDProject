...
CREATE_ORDER = 'CREATE TABLE IF NOT EXISTS ' + DBStructure.ORDER_TABLE_NAME + ' (' \
               + DBStructure.ORDER_TABLE_FIELD_ID + ' integer PRIMARY KEY AUTOINCREMENT,' \
...
CREATE_REFILL_STATION = 'CREATE TABLE IF NOT EXISTS ' + DBStructure.REFILL_STATION_TABLE_NAME + ' (' \
                        + DBStructure.REFILL_STATION_TABLE_FIELD_ID + ' integer PRIMARY KEY AUTOINCREMENT,' \
                        + DBStructure.REFILL_STATION_TABLE_FIELD_CHARGE_TIME + ' varchar(255) NOT NULL, ' \
                        + DBStructure.REFILL_STATION_TABLE_FIELD_ARRIVAL_TIME + ' DATETIME NOT NULL,' \
...
CREATE_CHARGE_EVENT = 'CREATE TABLE IF NOT EXISTS ' + DBStructure.CHARGE_EVENT_TABLE_NAME + ' (' \
                      + DBStructure.CHARGE_EVENT_TABLE_FIELD_LOCATION + ' varchar(255) NOT NULL,' \
                      + DBStructure.CHARGE_EVENT_TABLE_FIELD_ID + ' integer PRIMARY KEY AUTOINCREMENT,' \
                      + DBStructure.CHARGE_EVENT_TABLE_FIELD_TOTAL_AMOUNT_OF_CHARGERS + ' integer NOT NULL,' \
                      + DBStructure.CHARGE_EVENT_TABLE_FIELD_AMOUNT_OF_FREE_CHARGERS + ' integer NOT NULL,' \
...
# Many to many Client Location 0..N 1..N relation
# TODO what here is primary key
# TODO how to make 0..N _1..N_
CREATE_RELATION_CLIENT_LOCATION = 'CREATE TABLE IF NOT EXISTS ' + DBStructure.RELATION_CLIENT_LOCATION_NAME + ' (' \
                                  + DBStructure.RELATION_CLIENT_LOCATION_FIELD_LOCATION_ID + ' integer NOT NULL ,' \
                                  + DBStructure.RELATION_CLIENT_LOCATION_FIELD_CLIENT_USERNAME + ' varchar(255) NOT NULL' \
                                                                                                 ');'

# Many to many Manager Client 1..N 0..N
# TODO _1..N_ 0..N
CREATE_RELATION_MANAGER_CLIENT = 'CREATE TABLE IF NOT EXISTS ' + DBStructure.RELATION_MANAGER_CLIENT_NAME + ' (' \
                                 + DBStructure.RELATION_MANAGER_CLIENT_FIELD_CLIENT_USERNAME + ' varchar(255) NOT NULL ,' \
                                 + DBStructure.RELATION_MANAGER_CLIENT_FIELD_MANAGER_ID + ' integer NOT NULL' \
                                                                                          ');'
...
CREATE_STATE = 'CREATE TABLE IF NOT EXISTS ' + DBStructure.STATE_TABLE_NAME + ' (' \
               + DBStructure.STATE_TABLE_FIELD_ID + ' integer PRIMARY KEY AUTOINCREMENT, ' \
...
CREATE_ALL_CARS = 'CREATE TABLE IF NOT EXISTS ' + DBStructure.ALL_CARS_TABLE_NAME + ' (' \
                  + DBStructure.ALL_CARS_FIELD_ID + ' integer PRIMARY KEY AUTOINCREMENT, ' \
                  + DBStructure.ALL_CARS_FIELD_COLOR + ' varchar(255) NOT NULL, ' \
                  + DBStructure.ALL_CARD_FIELD_MANUFACTURE + ' varchar(255) NOT NULL, ' \
...
CREATE_PAYMENT = 'CREATE TABLE IF NOT EXISTS ' + DBStructure.PAYMENT_TABLE_NAME + ' (' \
                 ...
                 + DBStructure.PAYMENT_TABLE_FIELD_HASH_CREDIT_CARD + ' varchar(255) NOT NULL' \
                                                                      ');'
# Order - weak entity
# Order manager many to one
# CREATE_RELATION_ORDER_MANAGER = 'CREATE TABLE IF NOT EXISTS ' + DBStructure.RELATION_ORDER_MANAGER_NAME + ' (' \
#                                   + DBStructure.RELATION_ORDER_MANAGER_FIELD_MANAGER_ID + ' integer NOT NULL ,' \
#                                   + DBStructure.RELATION_ORDER_MANAGER_FIELD_ORDER_ID + ' integer NOT NULL' \
#                                                                                                  ');'
